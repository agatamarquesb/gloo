...
import { getTileColors } from '@/theme/colors';

import { useIsDarkMode } from './useIsDarkMode';

/**
 * The task-summary tile colors as literal hex values, for JS-side consumers that
 * can't use a Tailwind class (Recharts `fill`, inline `style`).
 *
 ...
 */
export function useTileColors(): string[] {
  ...
  return useMemo(() => getTileColors(), [isDark]);
}
